...
object LabeledPointParser {
  ...
  **/
  ...
  def GenLabeledPoint(
    ...
    val label_token:String = scala.io.Source.fromFile(_dataConfFile).getLines.toArray.filter(_.startsWith("@")).head
    ...
    label_type match {
      case "flag" => {
        ...
        val rstData = _spark.sparkContext.textFile(_inputFiles.head).map{
          x =>
          val splits = x.split('\t');
      ...
      case "repeat" =>{
        ...
        val data = _spark.sparkContext.textFile(_inputFiles.head).map{
          x =>
          val splits = x.split('\t');
      ...
      case "manual" =>{
        ...
        val aRstData = aData.map(line =>
          new LabeledPoint(0, Vectors.dense(featureMappor(fmc_b,idxs_b,line)).toSparse)
        ...
        val nRstData = nData.map(line =>
          new LabeledPoint(1, Vectors.dense(featureMappor(fmc_b,idxs_b,line)).toSparse)
          )
