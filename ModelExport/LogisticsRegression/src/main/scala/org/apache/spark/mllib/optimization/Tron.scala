...
//import breeze.linalg.{DenseVector,Vector=>BZ}
//
//
...
//
//  def gradient(dataPoints : RDD[(Double,Vector)], w : Vector, C : Double) : BZ[Double]
//
//  def hessianVector(dataPoints : RDD[(Double,Vector)], w : Vector, C : Double, s : BZ[Double]) : BZ[Double]
//}
//
///**
...
//  }
//
//  override def gradient(dataPoints : RDD[(Double,Vector)], w : Vector, C : Double) : BZ[Double] =
//  {
//    val n = w.size
//    val wBz = w.toBreeze
//    val g = dataPoints.mapPartitions(blocks => {
//      val grad = new DenseVector(Array.fill(n)(0.0))
//      for (p <- blocks)
//      {
//        val featureBz = p._2.toBreeze
//        val z = featureBz.dot(wBz)
...
//  }
//
//  override def hessianVector(dataPoints : RDD[(Double,Vector)], w : Vector, C : Double, sBz : BZ[Double]) : BZ[Double] =
//  {
//    val n = w.size
//    val wBz = w.toBreeze
//    val Hs = dataPoints.mapPartitions(blocks => {
//      var blockHs = new DenseVector(Array.fill(n)(0.0))
//      for (p <- blocks)
//      {
//        val featureBz = p._2.toBreeze
//        val z = featureBz.dot(wBz)
...
//
//
//  private def dnrm2_(v : BZ[Double]) : Double =
//  {
//    return math.sqrt(v.dot(v))
//  }
//
//  private def trcg(dataPoints : RDD[(Double,Vector)], C : Double, delta : Double, w : Vector, gBz : BZ[Double]) : (Int, BZ[Double], BZ[Double]) =
//  {
//    val n = w.size
//    //var (s, r, d) = (Vector.zeros(n), Vector.zeros(n), Vector.zeros(n))
//    var (rTr, rnewTrnew, beta, cgtol) = (0.0, 0.0, 0.0, 0.0)
//    var s = new DenseVector(Array.fill(n)(0.0))
//    var r = -gBz
//    var d = -gBz
...
//    val dataPoints = dataPointsIn.map(p=>(if(p._1>0.5)1.0 else -1.0, p._2)).cache
//    var w_new:BZ[Double] = new DenseVector(Array.fill(w.size)(0.0))
//    var wBz = w.toBreeze
//
//
//      /* Function Value*/
